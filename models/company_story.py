...
def _sentences(text: str, limit: int = 2, max_chars: int = 360) -> str:
    clean = _clean_text(text)
    if not clean:
        return ""
...
def _peer_context(peers: pd.DataFrame | None, dataset: dict) -> tuple[str, list[str]]:
    ...
    return _clip(f"Available peer set: {', '.join([label for label in labels if label])}.", 220), ["Peer comparison table"]
...
def build_company_story_summary(
    ...
    description = _sentences(dataset.get("company_description"), 2, max_chars=320)
    ...
    industry_positioning = _clip(f"Industry context: {sector} / {industry}. {peer_text}", 360)
    ...
    assumption_implications = [
        ...
        },
        {
            ...
            "implication": "Depends on moat durability, peer multiples, growth runway, margins, reinvestment needs, and industry structure.",
